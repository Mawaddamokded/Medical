...
function Inscription() {
  ...
  const register = () => {
    Axios.post("http://localhost:3000/cabinet/docteur/register", {
      nom_p: nomReg,
      prenom_p: prenomReg,
      login: loginReg,
  ...
  const conn = () => {
    Axios.post("http://localhost:3000/cabinet/docteur/login", {
      login: login,
      password: password,
    }).then((response) => {
  ...
  useEffect(() => {
    Axios.get("http://localhost:3000/cabinet/docteur/login").then((response) => {
